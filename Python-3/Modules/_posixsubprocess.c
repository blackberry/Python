...
#ifdef __QNX__
/* fork()/exec() support is poor on QNX. Use spawn() instead. */
#include <spawn.h>

static pid_t child_spawn(char *const exec_array[],
                       char *const argv[],
                       char *const envp[],
                       const char *cwd,
                       ...
                       PyObject *preexec_fn_args_tuple)
{
    ...
    fd_map = PyMem_Malloc(sizeof(int)*max_fd);
    if (fd_map == NULL) {
        PyErr_NoMemory();
        return -1;
    }
    ...
    for(i=0; i<fd_count; i++) {
        /* default to passing in all non-CLOEXEC fds */
    ...
    if(p2cwrite != -1)
        fd_map[p2cwrite] = SPAWN_FDCLOSED;
    if(c2pread != -1)
        fd_map[c2pread] = SPAWN_FDCLOSED;
    if(errread != -1)
        fd_map[errread] = SPAWN_FDCLOSED;

    fd_map[errpipe_read] = SPAWN_FDCLOSED;

    if(p2cread != -1)
        fd_map[0] = p2cread; /* stdin */
    if(c2pwrite != -1)
        fd_map[1] = c2pwrite; /* stdout */
    if(errwrite != -1)
        fd_map[2] = errwrite; /* stderr */

    if(p2cread > 2)
        fd_map[p2cread] = SPAWN_FDCLOSED;
    if(c2pwrite > 2)
        fd_map[c2pwrite] = SPAWN_FDCLOSED;
    if(errwrite != c2pwrite && errwrite > 2)
        fd_map[errwrite] = SPAWN_FDCLOSED;
    ...
    if(close_fds) {
        ...
        for (keep_seq_idx = 0; keep_seq_idx < num_fds_to_keep; ++keep_seq_idx) {
            ...
            if (keep_fd < 0) {  /* Negative number, overflow or not a Long. */
                PyErr_SetString(PyExc_ValueError, "bad value in fds_to_keep");
                goto error;
            }
            if (keep_fd < start_fd)
                continue;
            for (fd_num = start_fd; fd_num < keep_fd; ++fd_num) {
                fd_map[fd_num] = SPAWN_FDCLOSED;
            }
            start_fd = keep_fd + 1;
        }
        if (start_fd <= max_fd) {
    ...
    if(cwd) {
        ...
        if(chdir(cwd) < 0) {
            PyErr_SetFromErrnoWithFilename(PyExc_OSError, cwd);
            goto error;
        }
    }
    ...
    if(restore_signals) {
        ...
        /* from _Py_RestoreSignals */
        #ifdef SIGPIPE
            sigaddset(&inherit.sigdefault, SIGPIPE);
        #endif
        #ifdef SIGXFZ
            sigaddset(&inherit.sigdefault, SIGXFZ);
        #endif
        #ifdef SIGXFSZ
            sigaddset(&inherit.sigdefault, SIGXFSZ);
        #endif
    }
    ...
    for (i = 0; exec_array[i] != NULL; ++i) {
        ...
        pid = spawnp(executable, fd_count, fd_map, &inherit, argv, envp);
        if(pid >= 0) {
            saved_errno = 0;
            break;
        }
    ...
    /* Report the first spawn error, not the last. */
    if(pid == -1 && !saved_errno) {
        saved_errno = errno;
    }

    /* Don't bother checking for errors here: if it fails, we can't really
...
#ifdef __QNX__
        PyErr_SetString(PyExc_ValueError, "preexec_fn not supported on QNX");
        return NULL;
#else
        PyObject *result;
...
#ifdef __QNX__
    pid = child_spawn(exec_array, argv, envp, cwd,
                   p2cread, p2cwrite, c2pread, c2pwrite,
                   errread, errwrite, errpipe_read, errpipe_write,
